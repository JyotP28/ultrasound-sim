...
window.updateMod3Freq = function(freq) {
    ...
    if (textTarget) textTarget.innerHTML = `Probe: Curvilinear<br>Freq: ${f.toFixed(1)} MHz`;

    // THE FIX: Strict Step-by-Step Evaluation!
    if (window.Tutorial && window.Tutorial.evaluateAction) {
        if (window.Tutorial.currentStep === 1 && f >= 11.5) {
            window.Tutorial.evaluateAction('freq_high');
        }
        if (window.Tutorial.currentStep === 2 && f <= 3.5) {
            window.Tutorial.evaluateAction('freq_low');
        }
    }
};